...
class SsfmGPNp:
    ...
    def step(self):
        ...
        self.psi = rTimeEvo * ifft2(psik)
        self.nR = (
            np.exp(-(self.gamma + self.R * normSqr(self.psi)) * self.dt) * self.nR
            + self.pump * self.dt
        )
        self.t += self.dt
...
class DirSim:
    ...
    def __init__(self, psi0, start, end, dt):
        ...
        blockOne = (
            np.diag(np.full(self._samples, -3))
            + np.diag(np.full(self._samples - 1, 0.5), 1)
            + np.diag(np.full(self._samples - 1, 0.5), -1)
        )

        blockTwo = (
            np.diag(np.full(self._samples, 0.5))
            + np.diag(np.full(self._samples - 1, 0.25), -1)
            + np.diag(np.full(self._samples - 1, 0.25), 1)
        )

        offDiagTemplate = np.diag(np.full(self._samples - 1, 1), -1) + np.diag(
            np.full(self._samples - 1, 1), 1
        )
...
class SparseDirSim:
    ...
    def __init__(self, psi0, start, end, dt):
        ...
        self._h = (end - start) / self._samples
        blockOne = sps.diags(
            [0.5, -3, 0.5], offsets=[-1, 0, 1], shape=(self._samples, self._samples)
        )
        blockTwo = sps.diags(
            [0.25, 0.5, 0.25], offsets=[-1, 0, 1], shape=(self._samples, self._samples)
        )
        offDiagTemplate = sps.diags(
            [1, 0, 1], offsets=[-1, 0, 1], shape=(self._samples, self._samples)
        )
