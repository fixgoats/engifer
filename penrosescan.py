...
ndistances = 30
rad0 = pars["radius"]
rad1 = pars["radius"] - rstepsize * (ndistances - 1)
minsep = pars["radius"] / (goldenRatio**(pars["divisions"]+1))
rhomblength0 = minsep / (2 * np.sin(np.radians(18)))
rhomblength1 = rhomblength0 * rad1 / rad0
intermediaterun = pars["intermediaterun"]

siminfo = f'd{pars["divisions"]}'\
...
if args.use_cached is False:
    ...
    for j in range(ndistances):
        points = makeSunGrid(pars["radius"] - (j + 1) * rstepsize, pars["divisions"])
        pump = torch.zeros((samplesY, samplesX), dtype=torch.cfloat)
        for p in points:
            pump += pars["pumpStrength"]*gauss(gridX - p[0],
                                               gridY - p[1],
        ...
        np.save(f"tmp/intensity{j}.npy", sumd)
        bleh[:, j] = sumd
        fig, ax = figBoilerplate()
        ax.plot(dt * np.arange(nframes), npolars * dx * dy)
        name = f"penrosenr{pars['radius'] - (j+1) * rstepsize}{siminfo}"
        ...
        imshowBoilerplate(npnormSqr(rdata),
                          filename=f"penroserr{pars['radius'] - (j+1) * rstepsize}",
                          xlabel="x (µm)",
                          ylabel=r"y (µm)",
                          title=r"$|\psi_r|^2$",
        ...
        imshowBoilerplate(npnormSqr(kdata),
                          filename=f"penrosekr{pars['radius'] - (j+1) * rstepsize}",
                          xlabel="$k_x$ (µ$m^{-1}$)",
                          ylabel=r"$k_y$ (µ$m^{-1}$)",
                          title=r"$|\psi_k|^2$",
                          extent=[-kxmax/2, kxmax/2, -kymax/2, kymax/2])
        imshowBoilerplate(np.log(npnormSqr(kdata) + np.exp(-20)),
                          filename=f"penroseklogr{pars['radius'] - (j+1) * rstepsize}",
                          xlabel="$k_x$ (µ$m^{-1}$)",
                          ylabel=r"$k_y$ (µ$m^{-1}$)",
    ...
    for i in range(ndistances):
        sumd = np.load(f"tmp/intensity{i}.npy")
        bleh[:, i] = sumd

ommax = hbar * np.pi / dt
imshowBoilerplate(bleh[nframes//2-1:],
                  filename=f"penroseintensityr{pars['radius']}",
                  xlabel="d (rhombii side length) (µm)",
                  ylabel=r"E (meV)",
                  title=r"$I(E, d)$",
                  extent=[rhomblength0, rhomblength1, 0, ommax])
imshowBoilerplate(np.log(bleh[nframes//2-1:]),
                  filename=f"penroseintensitylogr{pars['radius']}",
                  xlabel="d (rhombii side length) (µm)",
                  ylabel=r"E (meV)",
                  title=r"$\ln(I(E, d))$",
                  extent=[rhomblength0, rhomblength1, 0, ommax])
